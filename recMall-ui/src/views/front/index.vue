...
<script>
import { listCategory, getCategory, delCategory, addCategory, updateCategory } from "@/api/mall/categories";
import { listNotice, getNotice, delNotice, addNotice, updateNotice } from "@/api/system/notice";
import { listBook, getBook, delBook, addBook, updateBook } from "@/api/mall/books";
...
export default {
  ...
  methods: {
    ...
    loadGoods() {
      return  listBook(this.queryParams).then(res => {
          if (res.code === '200') {
    ...
    loadType() {
      return listCategory(this.queryParams).then(res => {
        if (res.code === '200') {
          this.goodsData = res.rows;
        } else {
          this.$message.error(res.msg);
