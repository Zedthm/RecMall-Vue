...
<script>
import { listCategories, getCategories, delCategories, addCategories, updateCategories } from "@/api/mall/categories";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getCategories(categoryId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除书籍分类目录编号为"' + categoryIds + '"的数据项？').then(function() {
        return delCategories(categoryIds);
      }).then(() => {
        this.getList();
