...
<script>
import { listRel, getRel, delRel, addRel, updateRel } from "@/api/mall/rel";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加书籍与作者多对多关系";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const bookId = row.bookId || this.ids
      getRel(bookId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改书籍与作者多对多关系";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除书籍与作者多对多关系编号为"' + bookIds + '"的数据项？').then(function() {
        return delRel(bookIds);
      }).then(() => {
        this.getList();
