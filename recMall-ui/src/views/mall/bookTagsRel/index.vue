...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBookTagsRel(this.queryParams).then(response => {
        this.bookTagsRelList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加书籍与标签多对多关系";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const bookId = row.bookId || this.ids
      getBookTagsRel(bookId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改书籍与标签多对多关系";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.bookId != null) {
            updateBookTagsRel(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBookTagsRel(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除书籍与标签多对多关系编号为"' + bookIds + '"的数据项？').then(function() {
        return delBookTagsRel(bookIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('mall/bookTagsRel/export', {
        ...this.queryParams
