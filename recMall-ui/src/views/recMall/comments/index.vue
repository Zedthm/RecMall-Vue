...
export default {
  ...
  methods: {
    ...
    async del(id) {   // 单个删除
      try {
        await this.$confirm('您确定删除吗？', '确认删除', {type: "warning"});
        const res = await this.$request.delete('/comment/delete/' + id);
        ...
        this.$message.info('已取消删除操作'); // 用户取消操作的提示
      }
    },
    ...
    async delBatch() {   // 批量删除
      ...
      try {
        ...
        const res = await this.$request.delete('/comment/delete/batch', {data: this.ids});
        if (res.code === '200') {   // 操作成功
          this.$message.success('操作成功')
          this.load(1)
        } else {
